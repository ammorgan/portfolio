...
export default function About() {
  return <main id="top"><SiteNav />
    ...
    <section className="philosophy shell"><span className="eyebrow">Professional philosophy</span><div><p>I begin by understanding the current state: production behavior, repositories, APIs, business rules, technical documentation, and customer experiences.</p><p>From there, I partner with engineers, architects, and experience designers to challenge assumptions, simplify complexity, and create scalable solutions that preserve technical integrity while making the experience understandable and usable.</p><p>I lead with curiosity, empathy, and evidence—because I’m not driven by being right. I’m driven by finding the right answer.</p></div></section>
    <section className="principles shell"><div className="sectionHeading"><span className="eyebrow">What guides me</span><h2>I investigate the system <em>before defining the solution.</em></h2></div><div className="principleGrid">{principles.map(([title,text], index)=><article key={title}><span>0{index+1}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>
